...
export default function Step28() {
  ...
  const [selected, setSelected] = useState<string | null>(answers.personality3 ?? null);

  const choose = (id: string) => { setSelected(id); setAnswer("personality3", id); };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col flex-1 items-center w-full max-w-[664px] mx-auto px-4 sm:px-6 mt-2 mb-2">
        <SegmentedProgressBar filledCount={26} />
        <div className="flex-grow flex max-w-full" style={{ maxHeight: "calc(100dvh - 140px)" }}>
          <div className="flex flex-col flex-grow justify-center items-center max-w-full">
            ...
            <span className="font-semibold text-[26px] leading-[31px] text-black text-center font-[Montserrat,sans-serif] mt-6">
              I prefer opportunities to find me instead of me actively chasing them
            </span>
        ...
        <ContinueButton label="Next" disabled={!selected} onClick={() => selected && goToStep(29)} testId="next-button" />
      </div>
    </div>
  );
}
